...
import com.example.alarm.config.EmbeddedKafka;
import com.example.alarm.config.KafkaSseConsumer;
import com.example.alarm.config.KafkaSseProducer;
...
class AlarmKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/alarm-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/alarm-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/alarm-kafka/unregister"));
                return;
            }
        }
